...
import { Activity, AlertTriangle, BookOpen } from "lucide-react";
...
function SeverityBadge({
  severity,
}: { severity: CorrelationEntry["severity"] }) {
  const map = {
    high: "bg-destructive/10 text-destructive border-destructive/30",
    medium:
      "bg-orange-100 text-orange-700 border-orange-300 dark:bg-orange-950/40 dark:text-orange-400 dark:border-orange-700",
    low: "bg-muted text-muted-foreground border-border",
  };
  return (
    <span
      className={`inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium border ${map[severity]}`}
    >
...
export default function InjuryAnalysisUI({
  selectedRegions,
  onRegionsChange,
}: InjuryAnalysisUIProps) {
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          Select all body regions where injuries were sustained. The analysis
          will identify likely crash mechanisms based on established
      ...
      {selectedRegions.length > 0 && (
        <>
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Based on the selected injury locations, the following crash types
              are consistent with the injury pattern. Sorted by number of
              corroborating injury sites.
            </p>

            {sortedCrashTypes.length === 0 ? (
              <p className="text-sm text-muted-foreground italic">
                No correlations found for selected regions.
              </p>
            ) : (
            ...
            <div className="flex items-start gap-2 p-3 rounded-lg bg-muted/50 border text-xs text-muted-foreground">
              ...
              <p className="leading-relaxed">
                This analysis is based on established crash biomechanics
                research and is intended to assist with insurance documentation
                only. It does not constitute a medical diagnosis or legal
                determination of fault. Always seek medical attention for
                injuries and consult a qualified solicitor for legal advice.
              </p>
            </div>
          </div>
        </>
