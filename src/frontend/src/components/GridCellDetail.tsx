...
function FaultBar({
  ...
  const barBg =
    color === "red" ? "oklch(0.55 0.22 25)" : "oklch(0.45 0.18 250)";
  const textColor =
    color === "red" ? "oklch(0.45 0.22 25)" : "oklch(0.35 0.18 250)";
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            <div className="flex rounded-full overflow-hidden h-5 w-full">
              ...
              >
                {scenario.partyAFault >= 20
                  ? `${vehicleA.id}: ${scenario.partyAFault}%`
                  : ""}
              </div>
              <div
                className="flex items-center justify-center text-xs font-bold text-white transition-all duration-700"
              ...
              >
                {scenario.partyBFault >= 20
                  ? `${vehicleB.id}: ${scenario.partyBFault}%`
                  : ""}
              </div>
            </div>
      ...
      {scenarioRef && scenarioRef.highwayCode.length > 0 && (
        <Card>
          ...
          <CardContent className="space-y-2">
            {scenarioRef.highwayCode.map((ref) => (
              <div
                key={ref.rule}
