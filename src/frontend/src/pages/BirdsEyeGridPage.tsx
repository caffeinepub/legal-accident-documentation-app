...
import { Car, Grid3X3, Info, Plus, Trash2 } from "lucide-react";
...
function getFaultColor(fault: number): {
  bg: string;
  text: string;
  border: string;
} {
...
function MiniSplitBar({
  partyAFault,
  partyBFault,
}: { partyAFault: number; partyBFault: number }) {
  return (
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            <div className="flex items-center justify-between gap-1 mb-1">
              <span
                className={`text-xs font-bold px-1.5 py-0.5 rounded ${aColors.bg} ${aColors.text}`}
              >
            ...
            <div className="flex items-center justify-between gap-1">
              <span
                className={`text-xs font-bold px-1.5 py-0.5 rounded ${bColors.bg} ${bColors.text}`}
              >
                {vehicleB.label}: {scenario.partyBFault}%
              </span>
            </div>
            <MiniSplitBar
              partyAFault={scenario.partyAFault}
              partyBFault={scenario.partyBFault}
            />
        ...
        <TooltipContent side="top" className="max-w-xs">
          ...
          <p className="text-xs text-muted-foreground">
            {scenario.rationale.slice(0, 100)}…
          </p>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
...
export default function BirdsEyeGridPage() {
  ...
  const addVehicle = () => {
    ...
    setVehicles((prev) => [
      ...prev,
      { id: nextLabel, label: `Vehicle ${nextLabel}` },
    ]);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3 p-4 rounded-lg bg-muted/60 border border-border text-sm text-muted-foreground">
        ...
        <p>
          Fault percentages are indicative insurer assessments based on UK law
          and may vary by specific circumstances. This tool is for educational
                  ...
                  {faultMatrix.map((scenario) => (
                    ...
                    >
                      <GridCell
                        ...
                        scenario={scenario}
                        onClick={() =>
                          handleCellClick(vehicleA, vehicleB, scenario)
                        }
                      />
                    </td>
                  ))}
