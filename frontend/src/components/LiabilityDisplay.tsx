...
export default function LiabilityDisplay({ party1Liability, party2Liability }: LiabilityDisplayProps) {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Fault determination analysis pending</CardDescription>
        </CardHeader>
        <CardContent>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Fault determination based on detected violations and evidence</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2 p-4 bg-muted/50 rounded-lg">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Party 1 Liability</p>
            <p className={`text-3xl font-bold ${getTextColorClass(party1Percent)}`}>
              {party1Percent}%
            </p>
            <p className="text-xs text-muted-foreground">
              {party1Percent >= 70 ? 'Primary fault' : party1Percent >= 50 ? 'Shared fault' : 'Minor fault'}
            </p>
          </div>
          <div className="space-y-2 p-4 bg-muted/50 rounded-lg">
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Party 2 Liability</p>
            <p className={`text-3xl font-bold ${getTextColorClass(party2Percent)}`}>
              {party2Percent}%
            </p>
            <p className="text-xs text-muted-foreground">
              {party2Percent >= 70 ? 'Primary fault' : party2Percent >= 50 ? 'Shared fault' : 'Minor fault'}
            </p>
          </div>
        </div>
        ...
        <div className="p-3 bg-muted/30 rounded-lg border border-border">
          <p className="text-xs text-muted-foreground">
            <strong>Note:</strong> Liability percentages are calculated based on detected traffic violations, 
            road conditions, and evidence analysis. Higher percentages indicate greater responsibility for the incident.
          </p>
        </div>
      </CardContent>
    </Card>
