...
export default function InjuryAnalysisPanel({ reportId }: InjuryAnalysisPanelProps) {
  ...
  const handleDamagePhotosSelected = (_blobs: ExternalBlob[], files: File[]) => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      try {
        ...
        setStorageError('Analysis complete but could not save to backend. Please try again.');
      }
  ...
  const handleStore = async () => {
    ...
    try {
      const photoData: Array<[ExternalBlob, string, string]> = injuryPhotoBlobs.map((blob, i) => [
        blob,
      ...
      setStorageError(err instanceof Error ? err.message : 'Failed to store photos.');
    }
  };
        ...
        <button className="w-full flex items-center justify-between p-4 bg-card border border-border rounded-lg hover:bg-muted/50 transition-colors">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h3 className="font-semibold text-foreground">Injury &amp; Damage Photo Analysis</h3>
        ...
        <div className="mt-2 p-4 bg-card border border-border rounded-lg space-y-6">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mb-3">
                  Upload photos of visible injuries (bruising, lacerations, swelling) to help
                  correlate with crash type.
                </p>
                <InjuryPhotoUpload
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="font-medium text-foreground">Vehicle Damage Photos</h4>
                  <Badge variant="outline" className="text-xs">
                    Optional
                  </Badge>
                </div>
                <p className="text-sm text-muted-foreground mb-3">
                  Upload photos of vehicle damage (dents, broken glass, deformation) to help
                  determine impact direction and severity.
                </p>
                <InjuryPhotoUpload
              ...
              <TooltipProvider>
                <Tooltip>
                  ...
                  {!hasPhotos && (
                    <TooltipContent>
                      <p>Upload at least one injury or vehicle damage photo to run analysis</p>
                    </TooltipContent>
                  )}
                </Tooltip>
              </TooltipProvider>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="p-3 bg-primary/5 border border-primary/20 rounded-md">
                  <p className="text-xs text-muted-foreground mb-1">Inferred Crash Type</p>
                  <p className="font-semibold text-sm text-foreground">
                    {aiResult.inferredCrashType}
                  </p>
                </div>
                <div className="p-3 bg-amber-500/5 border border-amber-500/20 rounded-md">
                  <p className="text-xs text-muted-foreground mb-1">Severity Assessment</p>
                  <p className="font-semibold text-sm text-foreground">{aiResult.severity}</p>
                </div>
                <div className="p-3 bg-muted/50 border border-border rounded-md">
                  <p className="text-xs text-muted-foreground mb-1">Photos Analysed</p>
                  <p className="font-semibold text-sm text-foreground">
                    {injuryPhotoFiles.length + damagePhotoFiles.length} photo
                    {injuryPhotoFiles.length + damagePhotoFiles.length !== 1 ? 's' : ''}
                  </p>
                </div>
              </div>
              ...
              <div className="p-4 bg-muted/30 border border-border rounded-md">
                ...
                <p className="text-sm text-foreground leading-relaxed">{aiResult.narrativeText}</p>
              </div>
              ...
              {injuryPhotoBlobs.length > 0 ? (
                ...
                <div className="flex gap-3">
                  <Button variant="outline" onClick={handleDispose} className="flex-1">
                    Start Over
                  </Button>
          ...
          {step === 'done' && (
            <div className="space-y-4">
              ...
              <Button variant="outline" onClick={handleDispose} className="w-full">
                Run New Analysis
              </Button>
            </div>
          )}
