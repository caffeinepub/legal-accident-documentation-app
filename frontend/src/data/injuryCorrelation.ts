    ...
    correlations: [
      {
        ...
        severity: 'high',
        reference: 'NHTSA FARS data; Yoganandan et al., Frontiers in Neurology 2018',
      },
      {
        ...
        severity: 'medium',
        reference: 'NHTSA Technical Report DOT HS 809 765; Mertz & Driscoll 2003',
      },
      {
    ...
    correlations: [
      {
        ...
        severity: 'high',
        reference: 'Quebec Task Force on Whiplash-Associated Disorders 1995; UK MedCo guidelines',
      },
      {
        crashType: 'HEAD_ON_COLLISION',
        explanation:
    ...
    correlations: [
      {
        ...
        severity: 'medium',
        reference: 'Crandall et al., Accident Analysis & Prevention 2000; Froom v Butcher [1976]',
      },
      {
        crashType: 'AIRBAG_DEPLOYMENT_INJURY',
        explanation:
    ...
    correlations: [
      {
        ...
        severity: 'high',
        reference: 'IIHS Side Impact Research 2020; Viano et al., SAE Technical Paper 1989',
      },
      {
    ...
    correlations: [
      {
        ...
        severity: 'high',
        reference: 'Kuppa et al., Stapp Car Crash Journal 2001; EuroNCAP knee-bolster assessment',
      },
      {
        crashType: 'HEAD_ON_COLLISION',
        explanation:
        ...
        severity: 'high',
        reference: 'NHTSA NCAP; Dischinger et al., Accident Analysis & Prevention 2004',
      },
      {
        crashType: 'SIDE_IMPACT_TBONE',
        explanation:
...
export function getCorrelationsForRegion(region: BodyRegion): CorrelationEntry[] {
  const entry = INJURY_CORRELATIONS.find((c) => c.region === region);
  return entry ? entry.correlations : [];
}

export function getCorrelationsForRegions(regions: BodyRegion[]): Map<CrashType, CorrelationEntry[]> {
...
export function getSimpleCorrelationsForRegions(regionIds: string[]): SimpleCrashCorrelation[] {
    ...
    if (entry) {
      for (const corr of entry.correlations) {
        ...
        if (!seen.has(key)) {
          seen.add(key);
          results.push({
            crashType: getCrashTypeInfo(corr.crashType)?.label ?? corr.crashType,
